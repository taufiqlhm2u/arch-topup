...
new class extends Component {
    ...
    {
        session()->flash('success', 'Pesanan berhasil! Silahkan lakukan pembayaran.');
        $this->reset(['accountId', 'serverId', 'email', 'selectedId']);
        $this->quantity = 1;
    }
}
?>

<form method="post" action="{{ route('order.store') }}" target="_blank" class="space-y-5">
    @csrf

    {{-- Flash Error --}}
    @if (session('success'))
        <div
            class="flex items-center gap-3 rounded-xl bg-red-500/10 border border-green-500/30 px-4 py-3 text-sm text-green-400">
    ...
    <div class="rounded-2xl bg-white/3 border border-white/[.07] p-6 backdrop-blur-sm">
        <div class="flex items-center gap-3 mb-5">
            <div
                class="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-sm font-bold shadow-lg shadow-indigo-500/40 shrink-0">
                1</div>
        ...
        <div class="space-y-4">
            <div>
                <flux:label class="text-xs font-semibold uppercase tracking-widest text-neutral-400 mb-2 block">ID Akun
                </flux:label>
                <flux:input wire:model.live="accountId" name="player_id" placeholder="Contoh: 19123456" />
                @error('accountId') <p class="text-xs text-red-400 mt-1.5">{{ $message }}</p> @enderror
            </div>

            @if ($game->server_id)
                <div>
                    <flux:label class="text-xs font-semibold uppercase tracking-widest text-neutral-400 mb-2 block">Server
                        ID</flux:label>
                    <flux:input wire:model.live="serverId" name="server_id" placeholder="Contoh: 5504" />
    ...
    <div class="rounded-2xl bg-white/3 border border-white/[.07] p-6 backdrop-blur-sm">
        <div class="flex items-center gap-3 mb-5">
            <div
                class="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center text-sm font-bold shadow-lg shadow-indigo-500/40 shrink-0">
                2</div>
            <h2 class="font-bold text-lg">Pilih Paket yang Ingin Anda Beli</h2>
        </div>

        {{-- Tabs --}}
        @if ($this->types->count() > 1)
            <div class="flex gap-2 mb-6 flex-wrap">
                <button type="button" wire:click="setTab('all')" @class([
                ...
                @foreach ($this->types as $type)
                    <button type="button" wire:click="setTab('{{ $type }}')" @class([
                        'px-4 py-1.5 rounded-full text-sm font-semibold border transition-all duration-200 cursor-pointer',
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            @foreach ($this->filtered as $pkg)
                <button type="button" wire:click="pick({{ $pkg->id }})" @class([
                    ...
                    @else
                        <svg class="w-9 h-9 shrink-0 drop-shadow-[0_2px_8px_rgba(99,102,241,0.5)] group-hover:scale-110 transition-transform duration-200"
                            viewBox="0 0 32 32" fill="none">
                            <polygon points="16,3 28,11 24,27 8,27 4,11" fill="url(#g{{ $pkg->id }})"
                                stroke="rgba(99,102,241,0.4)" stroke-width="1" />
                    ...
                    <div class="flex-1 min-w-0">
                        <p
                            class="text-sm font-bold text-white truncate leading-tight group-hover:text-indigo-100 transition-colors duration-200">
    ...
    <div class="rounded-2xl bg-white/3 border border-white/[.07] p-6 backdrop-blur-sm">
        ...
        <div class="space-y-4 mb-5">
            ...
            {{-- Email --}}
            <div>
                <flux:label class="text-xs font-semibold uppercase tracking-widest text-neutral-400 mb-2 block">Email
                </flux:label>
                <div class="flex gap-2">
                    <flux:input wire:model.live="email" name="email" type="email" placeholder="[EMAIL_ADDRESS]"
                        class="flex-1" />
                </div>
                @error('email') <p class="text-xs text-red-400 mt-1.5">{{ $message }}</p> @enderror
                <p class="text-xs text-neutral-600 mt-2">Notifikasi pesanan akan dikirim ke email ini</p>
            </div>

            {{-- Quantity --}}
            <div>
                <flux:label class="text-xs font-semibold uppercase tracking-widest text-neutral-400 mb-2 block">Jumlah
                </flux:label>
                <div class="flex items-center gap-3">
                    <button type="button" wire:click="decrement" @class([
                    ...
                    <flux:input wire:model.live="quantity" readonly name="qty" type="number" min="1"
        ...
        <div class="rounded-xl bg-neutral-800/50 border border-white/5 p-4 mb-5 text-sm space-y-2.5">
            ...
            <div class="flex justify-between">
                ...
                <span class="font-semibold text-white">
                    ...
                    <input type="hidden" name="package_id" value="{{ $this->selected ? $this->selected->id : '' }}">
                </span>
            </div>
            <div class="flex justify-between">
        ...
        {{-- Submit --}}
        <flux:button type="submit" wire:loading.attr="disabled" variant="primary" :disabled="! $this->isReady"
            class="w-full" wire.click="placeOrder">
